...
/* Created on:     24.12.2015 23:26:54                          */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('Uporabnik') and o.name = 'FK_UPORABNI_UPORABNIK_IZBRISAN')
alter table Uporabnik
   drop constraint FK_UPORABNI_UPORABNIK_IZBRISAN
go
...
if exists (select 1
           ...
           where  id    = object_id('Uporabnik')
            and   name  = 'uporabnik_ima_izbrisan_racun_FK'
            and   indid > 0
            and   indid < 255)
   drop index Uporabnik.uporabnik_ima_izbrisan_racun_FK
go
...
create table Dogodek (
   ...
   nazivLokacije        varchar(100)         not null,
   opis                 varchar(1024)        not null,
   constraint PK_DOGODEK primary key nonclustered (id2)
)
go
...
create table PostavkaDogodka (
   ...
   opisCasaTrajanja     varchar(30)          not null,
   polniOpis            varchar(1024)        not null,
   constraint PK_POSTAVKADOGODKA primary key nonclustered (id2, idPostavkaDogodka)
)
go
...
create table SeznamDogodkov (
   ...
   constraint PK_SEZNAMDOGODKOV primary key nonclustered (uprabniskoIme, id2, idSeznamDogodkov)
)
go
...
create table Uporabnik (
   uprabniskoIme        varchar(100)         not null,
   idNaslov             int                  not null,
   Izb_uprabniskoIme    varchar(100)         null,
...
/*==============================================================*/
/* Index: uporabnik_ima_izbrisan_racun_FK                       */
/*==============================================================*/
create index uporabnik_ima_izbrisan_racun_FK on Uporabnik (
Izb_uprabniskoIme ASC
)
go
...
alter table PostavkaDogodka
   add constraint FK_POSTAVKA_DOGODEK_I_DOGODEK foreign key (id2)
      references Dogodek (id2)
go
...
alter table SeznamDogodkov
   add constraint FK_SEZNAMDO_DOGODEK_I_DOGODEK foreign key (id2)
      references Dogodek (id2)
go
...
alter table Uporabnik
   add constraint FK_UPORABNI_UPORABNIK_IZBRISAN foreign key (Izb_uprabniskoIme)
      references IzbrisanRacun (uprabniskoIme)
go
